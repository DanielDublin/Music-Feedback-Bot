...
class User_listener(commands.Cog):
    ...
    async def on_member_join(self, member: discord.Member):
        await self.bot.db.add_user(str(member.id))

        kicks = await self.bot.db.fetch_kicks(str(member.id))
        if kicks:
    ...
    # User was banned — remove from DB
    @commands.Cog.listener()
    ...
    async def handle_warnings(self, ctx, is_embed = False):
        ...
        warnings = await self.bot.db.add_warning_to_user(str(target_user.id))
    ...
    async def handle_wrong_prefix(self, message):
        ...
        try:
            ...
            logger.warning("Could not delete wrong-prefix message", exc_info=True)

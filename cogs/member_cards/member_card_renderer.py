...
_BASE_MEDIA_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), "../../media/Bebas_Neue/"))
FONT_PATH = os.path.join(_BASE_MEDIA_DIR, "BebasNeue-Regular.ttf")
BACKGROUND_IMAGES_DIR = os.path.join(_BASE_MEDIA_DIR, "images/")
BANNER_IMAGE_PATH = os.path.join(BACKGROUND_IMAGES_DIR, "banner.png")
...
def _get_star_points(center_x, center_y, num_points, outer_radius, inner_radius, rotation_degrees):
    points = []
...
def _draw_top_mfr_badge(draw_obj, y_pos, x_pos, font_top_mfr, icon_size, text_padding, text_color):
    star_color = (255, 215, 0)
    top_mfr_text = "TOP MFR"
    star_center_x = x_pos + icon_size // 2
    star_center_y = y_pos + icon_size // 2
    star_points = _get_star_points(star_center_x, star_center_y, 5, icon_size/2, icon_size/4, 0)
...
def get_text_bbox(text, font, max_width=None):
    font_size = font.size
...
def wrap_text(text, font, max_width):
    ...
    current_line = ""
    for word in words:
        bbox_word = get_text_bbox(word, font)
        ...
        if word_width > max_width:
            ...
            for char in word:
                ...
                temp_part_bbox = get_text_bbox(test_temp_part, font)
            ...
            test_bbox = get_text_bbox(test_line, font)
...
def generate_card(pfp_image_pil, discord_username, server_name, rank_str, numeric_points, message_count, join_date, card_size, font_path, animated=True, random_msg="", is_top_feedback=False, **kwargs):
    ...
    def setup_background():
        if rank_str == "Artist of the Week" and animated:
            ...
            if rank_str in BACKGROUND_MAP:
                background_filename = BACKGROUND_MAP[rank_str]
                background_image_path = os.path.join(BACKGROUND_IMAGES_DIR, background_filename)
    ...
    def process_roles():
        try:
            ...
            for role_name in all_genres_roles[:3]:
                displayed_roles_raw.append({'name': role_name, 'color': _hex_to_rgb("#8d8c8c")})
            for role_name in all_daws_roles[:3]:
                displayed_roles_raw.append({'name': role_name, 'color': _hex_to_rgb("#6155a6")})
            for role_name in all_instruments_roles[:3]:
                displayed_roles_raw.append({'name': role_name, 'color': _hex_to_rgb("#e3abff")})
            ...
            for tag_data in displayed_roles_final:
                ...
                try:
                    text_bbox = get_text_bbox(tag_text, font_tag)
    ...
    def process_random_message():
        ...
        while current_font_size >= random_msg_min_font_size:
            ...
            wrapped_message_temp = wrap_text(random_msg, temp_font, effective_text_width)
        ...
        if not font_found:
            ...
            if effective_text_width <= 0:
                effective_text_width = 1
            wrapped_message_temp = wrap_text(random_msg, final_font_random_msg, effective_text_width)
            ...
            if max_lines_for_min_font <= 0:
                ...
                if last_line_index >= 0:
                    ...
                    truncated_last_line = ""
                    ellipsis_bbox = get_text_bbox("...", final_font_random_msg)
                    ...
                    for char in original_last_line:
                        ...
                        test_bbox = get_text_bbox(test_line, final_font_random_msg)
                        test_width = test_bbox[2] - test_bbox[0]
        ...
        with Pilmoji(message_box) as pilmoji_draw:
            ...
            for i, line in enumerate(final_wrapped_message):
                ...
                temp_bbox = get_text_bbox(display_line, final_font_random_msg)
    ...
    def prepare_static_card():
        ...
        while True:
            dynamic_font_username = ImageFont.truetype(font_path, current_username_font_size)
            temp_bbox = get_text_bbox(temp_username_to_measure, dynamic_font_username)
            ...
            if current_username_font_size < username_min_font_size:
                ...
                while True:
                    ...
                    temp_bbox = get_text_bbox(test_str_with_ellipsis, dynamic_font_username)
                    temp_width = temp_bbox[2] - temp_bbox[0]
                    if temp_width <= max_username_width_area:
        ...
        username_bbox = get_text_bbox(final_username_to_draw, final_font_username)
        ...
        target_hex_color = RANK_BLINK_COLORS.get(rank_str, "#808080")
        rank_color_end = _hex_to_rgb(target_hex_color)
    ...
    if not animated:
        ...
        def render_static_card():
            ...
            if is_top_feedback:
                ...
                top_mfr_text_end_x, top_mfr_text_y = _draw_top_mfr_badge(static_draw, current_y_for_points_block, right_column_x_start, font_top_mfr, top_mfr_icon_size, top_mfr_text_padding, top_mfr_static_color)
    ...
    def render_animated_frames():
        ...
        for i in range(num_frames):
            ...
            if is_top_feedback:
                ...
                top_mfr_text_end_x, top_mfr_text_y = _draw_top_mfr_badge(
...
async def render_member_card(pfp_image_pil, discord_username, server_name, rank_str, numeric_points, message_count, join_date, card_size, font_path=None, animated=True, random_msg="", is_top_feedback=False, **kwargs):
    """Runs all Pillow work off the event loop. Returns (output, file_ext, log_collector)."""
    return await asyncio.to_thread(
        generate_card,
        pfp_image_pil, discord_username, server_name, rank_str, numeric_points,
        message_count, join_date, card_size, font_path or FONT_PATH,
        animated=animated, random_msg=random_msg, is_top_feedback=is_top_feedback,
        **kwargs
    )

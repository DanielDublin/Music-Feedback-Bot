...
**Fix:** Use `asyncio.to_thread()` (Python 3.9+, cleanest) or `run_in_executor`. Prefer `asyncio.get_running_loop()` over `asyncio.get_event_loop()` — the latter is only deprecated when called with *no* running event loop (Python 3.10+), but `get_running_loop()` is the explicit, correct call inside a coroutine and raises `RuntimeError` clearly if somehow called outside one.
```python
...
**Severity: Low** *(downgraded — see note)*
...
| A1  | `helpers.py:51` | `load_feedback_cog` implicit None return causes unpack crash | **Critical** |
| A2  | `user_listener.py:68` | `asyncio.sleep(86400)` in `on_message` — coroutine memory leak | **Critical** |
...
| A3  | `feedback_monitor.py` | `pending_validations` never cleared — unbounded memory growth | **Medium** |
| A4  | 3 files | Hardcoded `412733389196623879` instead of `CO_DEV_ID` | **Medium** |
| A5  | `mod_bad_feedback_notification.py:75` | Hardcoded channel ID instead of `FEEDBACK_ACCESS_CHANNEL_ID` | **Medium** |
| A6  | `user_listener.py:108` | Double audit log query in `on_member_remove` | **Medium** |
| A7  | `spotify_promotion_checker.py:67` | `if urls is None` never true — empty list passes through | **Medium** |
| A8  | `guild_events.py:48` | `fetch_user` called on every `submit` — not cached | **Medium** |
| B1  | `general.py` | `ctx: discord.Message` / `ctx: discord.Member` wrong type hints | **Medium** |
| B2  | `helpers.py:72,91,104` | Instance methods using `bot` as first param, missing `@staticmethod` | **Medium** |
| B3  | `database/db.py` | `user_id` key type mixed `int`/`str` in `users_dict` | **Medium** |
| C9  | `export_json.py:12` | Synchronous file I/O on event loop | **Low** *(small file, negligible impact)* |
| D1  | `helpers.py:50` | `fetch_channel` instead of `get_channel or fetch_channel` | **Medium** |
| B4  | `user_listener.py:26` | `on_message(self, ctx)` should be `(self, message: discord.Message)` | **Low** |
| B5  | Codebase-wide | Missing return type annotations on most functions | **Low** |
| A9  | `helpers.py:126` | `shorten_message` is `async` with no async work | **Low** |
| A10 | `helpers.py:114` | `add/remove_points_for_edits` are pointless single-line wrappers | **Low** |
| A11 | `finished_music_message.py` | Same message string hardcoded in two places | **Low** |
| A12 | `bot.py` | Unused variable, `IS_READY` int flag, unnecessary `global bot`, hardcoded guild ID | **Low** |
| A13 | `user_listener.py:1` | Unused imports `from re import T` and `from tarfile import NUL` | **Low** |
| D2  | `feedback_monitor.py:207` | Two API sends where one would work | **Low** |
...
| N11 | `database/db.py` | `add_user(called_from_update_func=True)` leaves `users_dict` empty — `KeyError` on first fetch | **High** |
...
**Severity: Critical** — any unhandled exception permanently kills the task
...
**Severity: High** — old pinned message is orphaned every time the bot restarts
...
**Severity: High** — `AttributeError` kills the loop on the very first iteration if `before_loop` fails
...
**Severity: High** — listener count grows indefinitely, eventually causing incorrect behaviour
...
**Severity: Medium** — task failures go completely unnoticed, causing apparent "task stop"
...
The `if messages_by_member_on_day:` check and the `random.choice` call are indented *inside* the `async for` loop body. On the first iteration where messages are found, `random.choice` runs immediately and returns — it never collects messages across multiple days before choosing.
...
**Fix:** Collect all candidate messages across the entire loop, then call `random.choice` *after* the loop exits.
...
`rank_options` is defined twice in the same file with identical content. `import datetime` also appears twice. The duplicates are dead code.
...
`ThreadsManager` is not a `commands.Cog` subclass. Calling `bot.add_cog` on it will raise `TypeError`. This function is never called from anywhere in the codebase — it is dead code.

**Fix:** Remove the `setup()` function.
...
Several embed-builder methods accept `thread` and/or `ctx` as parameters but never use them inside the method body. These dead parameters pollute every call site.

**Fix:** Remove the unused parameters from the method signatures and all call sites.
...
**Current state:** `guild_events.py`, `cogs/general.py`, and several other cogs each independently fetch and cache `self.pfp_url` with near-identical boilerplate in their `before_loop` / `cog_load` hooks.

**Recommendation:** Add a single helper on the bot instance (e.g. `bot.get_owner_pfp_url()`) that caches the URL after the first fetch. All cogs call this helper instead of managing their own cache.

---

### M2 — Wrap database access behind a `DatabaseService` class
**Current state:** Database calls are scattered across cogs via direct `db.fetch_rank(...)`, `db.update_points(...)` etc. `database/db.py` is a low-level connection-pool wrapper with no business logic layer.

**Recommendation:** Create `database/service.py` with a `DatabaseService` class that owns domain-specific methods (e.g. `get_user_rank`, `add_feedback_points`, `register_user`). Cogs receive a `DatabaseService` instance rather than the raw pool. This isolates SQL from cog code and makes testing straightforward.

---

### M3 — Define a clean API for `feedback_threads`
**Current state:** The feedback thread system spans `cogs/feedback_threads/feedback_threads.py`, `modules/threads_manager.py`, `modules/points_logic.py`, `modules/helpers.py`, and `modules/ctx_class.py`. External cogs (e.g. `general.py`) reach into `feedback_threads` by getting the cog and calling its internals directly.

**Recommendation:** Expose a small, stable public interface on `FeedbackThreads` cog (e.g. `get_user_thread`, `add_points`, `close_thread`) and have all external callers go through that interface only. Internal modules stay internal.

---

### M4 — Extract rank hierarchy into `data/constants.py`
**Current state:** Rank names, orderings, and role IDs appear as magic strings/numbers in `rank_commands.py`, `points_logic.py`, and `helpers.py`.

**Recommendation:** Define a single ordered list (or `enum`) in `data/constants.py`:
```python
RANK_ORDER = ["Bronze", "Silver", "Gold", "Platinum", "Diamond"]
RANK_ROLE_IDS = {"Bronze": 123, "Silver": 456, ...}
```
All rank-comparison and promotion logic reads from these constants.

---

### M5 — Split `get_member_card.py` / `member_class.py`
**Current state:** `member_class.py` mixes Discord data-fetching logic (message history, role checks) with image-generation logic (Pillow drawing calls).

**Recommendation:** Separate into:
- `member_data.py` — async Discord queries, returns a plain dataclass
- `member_card_renderer.py` — takes the dataclass, returns a `discord.File` (all Pillow calls, run in executor)

This also makes it straightforward to run the CPU-bound Pillow work off the event loop.

---

### M6 — Split `feedback_monitor.py` into monitor + notifier
**Current state:** `feedback_monitor.py` handles event listening, ML prediction, validation tracking, and notification logic all in one ~250-line class.

**Recommendation:** The file already has `mod_bad_feedback_notification.py` as a partial split. Complete it:
- `feedback_monitor.py` — event listening, prediction, pending_validations dict
- `mod_bad_feedback_notification.py` — already exists; expand to own all notification and reaction-wait logic
- `ml_model/ml_model_loader.py` — already separate; add `run_in_executor` wrapping here

---

### M7 — Make `ConfigureChannel` a cog-level dependency, not re-instantiated per command
**Current state:** Several slash commands instantiate a `ConfigureChannel` (or equivalent helper) object inline at the start of each handler.

**Recommendation:** Instantiate it once in `__init__` as `self.channel_config = ConfigureChannel(bot)` and reuse across commands. This avoids repeated object creation and keeps state consistent within a cog's lifetime.

---

### M8 — Extract `log_to_bot_log` into a shared logging utility
**Current state:** `log_to_bot_log` is copy-pasted with minor variations in `scan_delete_intro_messages.py`, `finished_music_message.py`, and `feedback_monitor.py`. All three versions are ~30 lines each and do the same thing: format a timestamp, build a code-block message, chunk it if > 2000 chars, send to `BOT_LOG`.

**Recommendation:** Create `utils/bot_logger.py` with a single `BotLogger` class or standalone `log_to_channel` coroutine. Each cog that needs it imports and calls it, passing `self.bot` and the log channel ID.
...
| ID | File | Description | Severity |
|----|------|-------------|----------|
| T1 | `database/db.py` | No `pool_recycle` — MySQL connections die after ~8h, killing all DB tasks | **Critical** |
| T2 | `finished_music_message.py` | No `.error` handler, no `reconnect=True` — any exception kills the task forever | **Critical** |
| T3 | `finished_music_message.py` | `stored_message_id` lost on restart — old messages orphaned | **High** |
| T4 | `scan_delete_intro_messages.py` | `self.channel` not initialised in `__init__` — `AttributeError` kills task | **High** |
| T5 | `music.py` | `NotesMenu` adds listener in `__init__` with no `remove_listener` — leak grows per invocation | **High** |
| T6 | `aotw_event.py` | `create_task` exceptions silently lost — task failures invisible | **Medium** |
| N1 | `rank_commands.py` | `update_rank_spreadsheet` receives `Role` object instead of role name string | **Critical** |
| N2 | `member_class.py` | Strategy 2 returns first day's message, not a random one across all days | **High** |
| N3 | `aotw_event.py` | `check_aotw_channel_announcement()` called twice; second call missing argument | **High** |
| N4 | `admin.py:185` | Module-level `print("Processing complete")` runs on every import | **Medium** |
| N5 | `rank_commands.py` | Duplicate `rank_options` list and duplicate `datetime` import | **Low** |
| N6 | `music.py` | `fetch_user` and `open('options.json')` called on every command | ~~**Medium**~~ ✅ Resolved |
| N7 | `threads_manager.py` | Dead `setup()` function registers a non-Cog as a Cog | **Low** |
| N8 | `embeds.py` | Unused `thread` / `ctx` params across multiple embed methods | **Low** |
| N9 | `member_class.py` | `get_message_count` is an unimplemented stub returning 0 | **Medium** |
| N10 | `helpers.py:72,91,104` | `bot` as first param on instance methods — `self` passed as `bot` at runtime | **Medium** |
...
**Severity: Medium** | `admin.py:25–28`
...
`default_permissions` and `@app_commands.checks.has_permissions()` serve *different* purposes and should coexist:
...
**Severity: High** | `admin.py`, `rank_commands.py`
...
1. `guild_only` is **verified server-side by Discord**, not as a `check`. The docstring states: *"there is no error handler called when a command is used within a private message."* This means your global `@bot.tree.error` handler will NOT fire for guild-only violations — Discord simply won't surface the command in DMs.
...
**Severity: Low** | `feedback_monitor.py`, `threads_manager.py`
...
**Severity: Low** | `bot.py:39`

`global` inside a function is only needed when you *assign* to the name. `on_ready` never reassigns `bot` — it only reads it. The `global bot` declaration is a no-op and misleads the reader into thinking `bot` is being modified.

---

## E10 — Discord API: No `@app_commands.describe()` on slash command parameters
**Severity: Low** | All slash command cogs
...
**Severity: Medium** | All slash command cogs
...
**Severity: Medium** | `music.py` (NotesMenu), `mod_bad_feedback_notification.py`
...
**Severity: Low** | `rank_commands.py`, genre/similar-bands commands
...
**Severity: Medium** | `google_sheet.py`
...
| ID  | File | Description | Severity |
|-----|------|-------------|----------|
| E1  | `bot.py:47–61` | Cog init in `on_ready` instead of `async def cog_load` | **High** |
| E2  | `bot.py:115–117` | Slash command global error handler sends raw errors, can crash on None channel | **High** |
| E3  | All cogs | No `cog_command_error` in any cog | **Medium** |
| E4  | `admin.py:25` | Manual permission check duplicates decorator logic | **Medium** |
| E5  | `admin.py`, `rank_commands.py` | No `@app_commands.guild_only()` — crashes in DMs | **High** |
| E6  | `bot.py:71` | Global `tree.sync()` on every boot — rate limit risk | ~~**Medium**~~ ✅ Resolved |
| E7  | Codebase-wide | `print()` instead of `logging` module | **Medium** |
| E8  | `feedback_monitor.py` etc. | Raw dicts for structured state instead of `dataclasses` | **Low** |
| E9 | `bot.py:39` | `global bot` is a no-op | **Low** |
| E10 | All slash cogs | No `@app_commands.describe()` on parameters | **Low** |
| E11 | All slash cogs | No `@app_commands.checks.cooldown` on any command | **Medium** |
| E12 | All task loops | `add_exception_type()` not called for DB/Sheets exceptions | **High** |
| E13 | `music.py`, `mod_bad_feedback_notification.py` | Raw reactions instead of `discord.ui.View` | **Medium** |
| E14 | All cogs | `interaction_check` not used for cog-wide guards | **Low** |
| E15 | `rank_commands.py` etc. | No autocomplete on known-value parameters | **Low** |
| E16 | — | No `/reload` command — must restart to apply changes | **Medium** |
| E17 | — | No `/status` / `/health` command | **Medium** |
| E18 | `google_sheet.py` | Individual Sheets writes not batched — quota risk | **Medium** |

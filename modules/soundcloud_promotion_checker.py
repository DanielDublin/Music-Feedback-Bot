...
def extract_soundcloud_song_name(expanded_url):
    # Regular expression to match the channel name in a SoundCloud URL
    soundcloud_url_pattern = re.compile(r'https?://soundcloud\.com/[\w-]+/([\w-]*)')
...
async def check_soundcloud(message):
    ...
    for url in short_urls:
        ...
        soundcloud_user_display_name = soundcloud_user_display_name.replace(" ","").replace("-","").replace("_","").lower()
        soundcloud_default_user_name = soundcloud_default_user_name.replace(" ","").replace("-","").replace("_","").lower()
        discord_global_name = author.global_name.replace(" ","").replace("-","").replace("_","").lower()
        discord_display_name = author.display_name.replace(" ","").replace("-","").replace("_","").lower()
        
        try:     
            # Check if the the Soundcloud username is in the name, nickname or bio of the discord's user
            if soundcloud_user_display_name == discord_global_name or soundcloud_user_display_name == discord_display_name:
                return True
            elif soundcloud_default_user_name == discord_global_name or soundcloud_default_user_name == discord_display_name:
                return True
